...
mod helpers; // contains the sort_files() function
...
fn run_report( marc_full_source_files_dir: &str, marc_full_output_files_dir: &str) {
    ...
    let unsorted_compressed_marc_files: Vec<std::path::PathBuf> = std::fs::read_dir(marc_full_source_files_dir)
        .expect("Unable to read directory")
        .map(|res| res.map(|e| e.path()))
        .collect::<Result<Vec<_>, std::io::Error>>()
        .expect("Unable to collect files");
    log_debug!( "first 3 unsorted_compressed_marc_files: {:?}", &unsorted_compressed_marc_files[0..3] );

    // get a sorted list --------------------------------------------
    let mut compressed_marc_files: Vec<std::path::PathBuf> = helpers::sort_files( unsorted_compressed_marc_files );
    log_debug!( "first 3 sorted_compressed_marc_files: {:?}", &compressed_marc_files[0..3] );
